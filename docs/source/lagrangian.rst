...
#. "x_res", "y_res", "z_res": These parameters control the degree on data reduction. We recommend using either values of 2, i.e., every other grid point in each direction or 1:8 data reduction, or 3, i.e., every third grid point in each direction or 1:27 data reduction. Setting each parameter to 4 would result in a 1:64 data reduction and would reduce data integrity. 
